...
import type { Subject, Subscription } from '../utils/create-subject';
...
export type ChangeHandler = (event: { target: any; type?: any }) => Promise<undefined | boolean>;
...
export type UseFormGetObservable<TFieldValues extends FieldValues> = {
    /**
     * Get the entire form values when no argument is supplied to this function.
     *
     * @remarks
     ...
     * ```
     */
    <TFieldName extends FieldPath<TFieldValues>>(name: TFieldName): Observable;
    /**
     * Get an array of field values.
     *
     * @remarks
...
> = {
    ...
    _updateFieldArray?: BatchFieldArrayUpdate; // TODO
    _getFieldArray?: <TFieldArrayValues>(name: InternalFieldName) => Partial<TFieldArrayValues>[]; // TODO
    ...
    _disableForm?: (disabled?: boolean) => void; // TODO
...
> extends Pick<
        ...
        'mode' | 'disabled' | 'reValidateMode' | 'defaultValues'
    > {
    resolver: (values: TFieldValues) => ReturnType<Resolver<TFieldValues, TTransformedValues>>;
...
> {
    ...
    getObservable: UseFormGetObservable<TFieldValues>;
    peekValues: UseFormGetValues<TFieldValues>;
